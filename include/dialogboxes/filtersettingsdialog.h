...
{
public:
    static INT_PTR CALLBACK FilterSettingsDialogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        FilterSettingsDialog *pThis = reinterpret_cast<FilterSettingsDialog*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));

        if (!pThis)
        {
            if (uMsg == WM_INITDIALOG)
            {
                pThis = reinterpret_cast<FilterSettingsDialog*>(lParam);
                pThis->m_hwnd = hwnd;
