...
{
public:
    static INT_PTR CALLBACK ShpTypeDialogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ShpTypeDialog *pThis = reinterpret_cast<ShpTypeDialog*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));

        if (!pThis)
        {
            if (uMsg == WM_INITDIALOG)
            {
                pThis = reinterpret_cast<ShpTypeDialog*>(lParam);
                pThis->m_hwnd = hwnd;
