...
{
public:
    static INT_PTR CALLBACK ImportDialogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ImportDialog *pThis = reinterpret_cast<ImportDialog*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));

        if (!pThis)
        {
            if (uMsg == WM_INITDIALOG)
            {
                pThis = reinterpret_cast<ImportDialog*>(lParam);
                pThis->m_hwnd = hwnd;
